...
export default function AdminApp() {
  ...
  // Login Screen
  if (!isLoggedIn) {
    return React.createElement('div', { className: "min-h-screen bg-gradient-to-br from-red-50 to-red-100 flex items-center justify-center p-4" },
      React.createElement('div', { className: "bg-white rounded-3xl shadow-2xl p-12 w-full max-w-md" },
        React.createElement('div', { className: "text-center mb-8" },
          React.createElement('div', { className: "w-20 h-20 bg-red-600 rounded-2xl mx-auto mb-6 flex items-center justify-center" },
            React.createElement('span', { className: "text-white font-black text-2xl" }, "DRK")
          ),
          React.createElement('h1', { className: "text-3xl font-black text-slate-900 tracking-tight" }, "Admin Login"),
          React.createElement('p', { className: "text-slate-500 font-medium mt-2" }, "DRK Melm Dashboard")
        ),
        React.createElement('form', { onSubmit: handleLogin, className: "space-y-6" },
          ...
          loginError && React.createElement('p', { className: "text-red-600 font-bold text-sm text-center" }, "Falsches Passwort"),
  ...
  return React.createElement('div', { className: "min-h-screen bg-slate-50" },
    ...
    status && React.createElement('div', { 
      className: `fixed top-4 right-4 z-50 flex items-center gap-3 px-6 py-4 rounded-2xl shadow-xl font-bold ${
        status.type === 'success' ? 'bg-green-600 text-white' : 'bg-red-600 text-white'
      }`
    }, status.message),

    // Header
    React.createElement('header', { className: "bg-white border-b border-slate-200 px-6 py-4" },
      React.createElement('div', { className: "max-w-6xl mx-auto flex items-center justify-between" },
        React.createElement('div', { className: "flex items-center gap-4" },
          React.createElement('div', { className: "w-12 h-12 bg-red-600 rounded-xl flex items-center justify-center" },
            React.createElement('span', { className: "text-white font-black" }, "DRK")
          ),
          React.createElement('div', null,
            React.createElement('h1', { className: "text-2xl font-black text-slate-900 tracking-tight" }, "Admin Dashboard"),
            React.createElement('p', { className: "text-slate-500 font-medium text-sm" }, "DRK Melm Verwaltung")
          )
        ),
        React.createElement('button', {
    ...
    // Main Content
    React.createElement('main', { className: "max-w-6xl mx-auto p-6 space-y-8" },
      // Save Button
      React.createElement('div', { className: "flex justify-end" },
        React.createElement('button', {
          onClick: saveData,
          disabled: isSaving,
          className: "px-8 py-4 bg-red-600 text-white font-black rounded-2xl hover:bg-red-700 transition-colors disabled:opacity-50 shadow-lg"
        }, isSaving ? 'Speichert...' : 'Änderungen Speichern')
      ),

      React.createElement('div', { className: "grid grid-cols-1 lg:grid-cols-3 gap-8" },
        // Weekly Program
        React.createElement('div', { className: "lg:col-span-2 bg-white rounded-3xl p-8 shadow-sm border border-slate-200" },
          React.createElement('h2', { className: "text-2xl font-black text-slate-900 mb-6" }, "📅 Wochenprogramm"),
          React.createElement('div', { className: "space-y-4" },
            weeklyProgram.map((entry, index) =>
              React.createElement('div', { key: entry.day, className: "grid grid-cols-6 gap-4 p-4 bg-slate-50 rounded-2xl" },
                React.createElement('div', { className: "font-black text-slate-900 text-lg flex items-center" }, entry.day),
                ...
                React.createElement('div', null,
                  React.createElement('select', {
                    ...
                    React.createElement('option', { value: "" }, "Ort wählen"),
        ...
        // Event Config
        React.createElement('div', { className: "bg-slate-900 text-white rounded-3xl p-8" },
          React.createElement('h2', { className: "text-xl font-black mb-6" }, "🎉 Event-Modus"),
          
          // Active Toggle
          React.createElement('div', { className: "mb-6" },
            React.createElement('label', { className: "text-sm font-bold text-white/70 block mb-2" }, "Status"),
            React.createElement('div', { className: "grid grid-cols-2 gap-2 bg-white/10 p-1 rounded-xl" },
              React.createElement('button', {
          ...
          React.createElement('div', { className: "space-y-4" },
            React.createElement('div', null,
              React.createElement('label', { className: "text-sm font-bold text-white/70 block mb-2" }, "Bilddatei"),
              React.createElement('input', {
                type: "text",
            ...
            React.createElement('div', null,
              React.createElement('label', { className: "text-sm font-bold text-white/70 block mb-2" }, "Start"),
              React.createElement('input', {
                type: "datetime-local",
            ...
            React.createElement('div', null,
              React.createElement('label', { className: "text-sm font-bold text-white/70 block mb-2" }, "Ende"),
              React.createElement('input', {
                type: "datetime-local",
