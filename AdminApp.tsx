      ...
      <div className="min-h-screen flex items-center justify-center p-4 bg-slate-900">
        <div className="bg-white w-full max-w-md rounded-[3rem] shadow-2xl p-12 text-center animate-in zoom-in duration-500">
          ...
          <form onSubmit={(e) => { e.preventDefault(); password === ADMIN_PASSWORD ? setIsLoggedIn(true) : setLoginError(true); }} className="space-y-6">
            ...
            />
            {loginError && <p className="text-red-600 font-black text-[10px] uppercase">Falsches Passwort</p>}
  ...
  return (
    <div className="min-h-screen bg-[#f1f5f9]">
          ...
          <div className="bg-white w-full max-w-xl rounded-[3.5rem] shadow-2xl overflow-hidden animate-in zoom-in duration-300">
            ...
            <div className="p-12 space-y-8">
              <div className="space-y-4">
                ...
                <input type="password" value={githubConfig.token} onChange={e => setGithubConfig(p => ({...p, token: e.target.value}))} className="w-full px-8 py-5 bg-slate-50 border-2 border-slate-200 rounded-2xl font-black outline-none" placeholder="GitHub Token" />
              </div>
      ...
      <main className="max-w-7xl mx-auto p-10 space-y-12">
        {statusMsg && (
          <div className={`fixed top-28 left-1/2 -translate-x-1/2 z-[110] px-12 py-6 rounded-full shadow-2xl flex items-center gap-6 border-4 ${statusMsg.type === 'success' ? 'bg-emerald-600 border-emerald-400 text-white' : 'bg-red-600 border-red-400 text-white'}`}>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-4 gap-8">
          <div className="lg:col-span-3 bg-white p-8 rounded-[3.5rem] border border-slate-200 flex items-center justify-between shadow-sm">
            ...
            <div className="flex items-center gap-4">
              <button onClick={() => setCurrentWeekOffset(p => p - 1)} className="p-5 bg-slate-50 rounded-2xl hover:text-red-600 transition-all border border-slate-100 active:scale-95"><ChevronLeft size={32}/></button>
              <button onClick={() => setCurrentWeekOffset(p => p + 1)} className="p-5 bg-slate-50 rounded-2xl hover:text-red-600 transition-all border border-slate-100 active:scale-95"><ChevronRight size={32}/></button>
            </div>
          </div>
          
          <button onClick={saveAllToGitHub} disabled={isSyncing} className={`rounded-[3.5rem] font-black uppercase tracking-widest text-white shadow-2xl flex flex-col items-center justify-center gap-3 transition-all ${isSyncing ? 'bg-slate-400 scale-95' : 'bg-red-600 hover:bg-red-700 active:scale-95 shadow-red-600/20'}`}>
            {isSyncing ? <Loader2 className="animate-spin" size={40}/> : <Cloud size={40}/>}
            <span className="text-xs">Speichern & Live</span>
          </button>
        </div>
        ...
        <div className="grid grid-cols-1 xl:grid-cols-3 gap-12">
                ...
                return (
                  <div key={day} className="group p-6 rounded-[2rem] hover:bg-slate-50 flex items-center gap-6 transition-all">
                    ...
                    <div className="flex-1 relative">
                      ...
                      <button onClick={() => handleAISuggestion(idx)} className="absolute right-4 top-1/2 -translate-y-1/2 text-slate-300 hover:text-red-600 p-2">
                        {loadingSuggestion === id ? <Loader2 className="animate-spin" size={24}/> : <Sparkles size={24}/>}
                      </button>
                    </div>
                    <select value={data.location} onChange={e => updateActivity(idx, 'location', e.target.value)} className="w-48 bg-white border-2 border-slate-100 rounded-[1.5rem] px-6 py-5 font-bold outline-none shadow-sm focus:border-red-600 cursor-pointer">
                      <option value="">Ort...</option>
                      {LOCATIONS.map(l => <option key={l} value={l}>{l}</option>)}
                    </select>
                    <input type="time" value={data.time} onChange={e => updateActivity(idx, 'time', e.target.value)} className="w-36 bg-white px-6 py-5 rounded-[1.5rem] border-2 border-slate-100 font-black text-lg outline-none focus:border-red-600 shadow-sm" />
                  </div>
                );
              })}
            </div>
          ...
          <section className="bg-slate-900 text-white rounded-[3.5rem] p-10 shadow-2xl space-y-8 border border-white/5 h-fit sticky top-32">
             ...
             <div className="grid grid-cols-2 gap-2 bg-white/5 p-1 rounded-2xl border border-white/10">
               <button onClick={() => setEventConfig(p => ({...p, active: true}))} className={`py-4 rounded-xl font-black text-[10px] uppercase tracking-widest transition-all ${eventConfig.active ? 'bg-white text-slate-900 shadow-md' : 'text-white/30 hover:text-white/60'}`}>An</button>
               <button onClick={() => setEventConfig(p => ({...p, active: false}))} className={`py-4 rounded-xl font-black text-[10px] uppercase tracking-widest transition-all ${!eventConfig.active ? 'bg-red-600 text-white shadow-md' : 'text-white/30 hover:text-white/60'}`}>Aus</button>
             </div>
